...
public class DoubleArraySequence implements Cloneable {
    ...
    public void addBefore(double element) {
        int start = currentIndex != manyItems ? currentIndex : 0;
        int newLength = manyItems + 1 > data.length ? data.length * 2 : data.length;
    ...
    public static DoubleArraySequence catenation(DoubleArraySequence s1, DoubleArraySequence s2) {
        DoubleArraySequence result = new DoubleArraySequence(s1.manyItems + s2.manyItems);
        int manyItems = s1.manyItems + s2.manyItems;
        result.manyItems = manyItems;

        for (int i = 0; i < s1.manyItems; i++) {
            result.data[i] = s1.data[i];
        }
        for (int j = 0; j < s2.manyItems; j++) {
            result.data[s1.manyItems + j] = s2.data[j];
        }

        result.currentIndex = manyItems;
